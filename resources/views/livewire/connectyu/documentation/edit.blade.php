<section class="text-gray-700 body-font bg-green-200">
    <div class="container px-8 py-2 mx-auto lg:px-4">
        <div class="flex flex-col w-full mb-2 text-left lg:text-center">
            ...
            <form method="post" action="/documentations/{{$documentation->id}}" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="w-full p-2">
                    <label class="block ">
                        <label for="email" class="text-sm leading-7 text-gray-600">Choose</label>
                        <select name="device" value="{{$documentation->device}}"
                ...
                <div class="w-full p-2" >
                    <input type="text"
                           id="password" name="topic" placeholder="topic of your tutorial" value="{{$documentation->topic}}"
                           class="w-full px-4 py-2 mr-4 text-base text-blue-700 bg-gray-100 border-transparent rounded-lg focus:border-gray-
                       500 focus:bg-white focus:ring-0">
                </div>
